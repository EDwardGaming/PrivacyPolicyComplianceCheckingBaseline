...
class BILSTM_Model(object):
    def __init__(self, vocab_size, out_size, crf=True):
        ...
        self.dropout = LSTMConfig.dropout

        self.crf = crf
        # 根据是否添加crf初始化不同的模型 选择不一样的损失计算函数
        if not crf:
            self.model = BiLSTM(vocab_size, self.emb_size,
                                self.hidden_size, out_size, self.dropout).to(self.device)
...
class BiLSTM_CRF(nn.Module):
    def __init__(self, vocab_size, emb_size, hidden_size, out_size, dropout):
        """初始化参数：
            vocab_size:字典的大小
            emb_size:词向量的维数
            hidden_size：隐向量的维数
        ...
        self.bilstm = BiLSTM(vocab_size, emb_size, hidden_size, out_size, dropout)
