...
def gptner_train_and_eval(test_data, remove_O=False):
    test_word_lists, test_tag_lists = test_data
    rankgpt_model = GPTNER()

    pred_tag_lists = []

    # <--- 2. 修改这里的循环，加上 tqdm 包装器
    # desc="GPT预测中" 是进度条左侧显示的文字，可以自定义
    for i, words in enumerate(tqdm(test_word_lists, desc="GPT预测中", unit="sents")):
        pred_tags = rankgpt_model.predict(words, start_index=i+1)
